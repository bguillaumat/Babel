...
DecodedSound	Opus::decodeSound(EncodedSound &soundEncode) {
	...
	decodeSound.size = opus_decode_float(_decoder, soundEncode.buffer.data(), soundEncode.size, decodeSound.buffer.data(), SoundDeviceSetting::sampleRate, 0) * SoundDeviceSetting::channels;
	return decodeSound;
}
